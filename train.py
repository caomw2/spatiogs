...
from utils.loss_utils import l1_loss, ssim, FARC_loss, entropy_loss
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        for viewpoint_cam in viewpoint_cams:
            if iteration >= opt.keypoint_from_iter and iteration % opt.keypoint_add_interval == 0 and iteration < 5001 and stage == "fine" and gaussians.get_xyz.shape[0]<220000:
                # stage = "Keypoint"
                time1_xyz = gaussians.get_xyz_bytime(0.3).detach()
                time2_xyz = gaussians.get_xyz_bytime(0.5).detach()
                time3_xyz = gaussians.get_xyz_bytime(0.7).detach()
            ...
            # For computing FARC
            if iteration > opt.farc_loss_until and opt.farc_loss_use == True:
                ...
                if cluster_centers_indices is None:
                    ...
                    for i, center in enumerate(cluster_centers):
                        cluster_points_indices = np.where(cluster_indices == i)[0]
                    ...
                    # Step 5
                farcloss += FARC_loss(means_3D_deform_tensor, opt.knn_neighbor_num, lambda_knn_weight = opt.lambda_knn_weight, cluster_centers_indices = cluster_centers_indices)
            # For computing entropy loss
            if opt.entropy_loss_weight != 0:
                opacities_tensor = render_pkg["opacities"]
                entropyloss += opt.entropy_loss_weight * entropy_loss(opacities_tensor) + opt.entropy_loss_weight * entropy_loss(gaussians.get_opacity)
            # For guide pruning EPG
            if iteration > opt.pruning_from_iter and iteration % opt.pruning_interval == 0 and gaussians.get_xyz.shape[0]>200000 and iteration < opt.densify_until_iter and opt.guide_pruning_use == True:
                if psnr(image.clone().detach(), gt_image.clone().detach()) < opt.error_psnr_threshold :
        ...
        loss = farcloss/len(viewpoint_cams)*opt.knn_loss_weight + loss if farcloss > 0 else loss
