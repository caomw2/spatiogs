...
from utils.external import o3d_farc_loss, compute_angle_between_vectors
...
class GaussianModel:
    ...
    def prune(self, max_grad, min_opacity, extent, max_screen_size, final_error_points_mask = None):
        ...
        if max_screen_size:
            ...
            prune_mask = torch.logical_or(prune_mask, final_error_points_mask) if final_error_points_mask is not None else prune_mask
    ...
    def error_guide_add_points(self, selected_pts, perturb=1, num_samples_per_point = 2):
        ...
        neighbors_mask = o3d_farc_loss(all_point, query_pts, 1)
    ...
    def get_xyz_bytime(self, time):
        ...
        return xyz.clone().detach().cpu()
    
    def create_keypoint(self, point_tensor1, point_tensor2, point_tensor3):
        velocity = (point_tensor2 - point_tensor1) / 0.2  # 速度 = 位移 / 时间间隔
        velocity2 = (point_tensor3 - point_tensor2) / 0.2 # 第二个时间点到第三个时间点的速度
        ...
        keypoint_coords = point_tensor1[self.keypoint].cpu().numpy()
        keypoint_pcd = o3d.geometry.PointCloud()
        keypoint_pcd.points = o3d.utility.Vector3dVector(keypoint_coords)
        o3d.io.write_point_cloud("keypoint.ply", keypoint_pcd)